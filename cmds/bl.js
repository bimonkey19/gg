  ...
  return false;
    ...
    (
        ...
        for (i=0; i < online ; i ++ )
        {
            ...
            if (list.length > 900) {
              listnamebl[pagebl++] = list;
              var list = '';
            };

          if (i == online - 1)
          {
            if (list) listnamebl[pagebl] = list;
            if (!listnamebl[pagebl])  pagebl--;
          }

        }
        var tengang1ne = 'BLOOD'
          ...
          .setColor('RANDOM')
          .addFields(
              { name: `${tengang1ne} Online: `+slbl    , value: '➖➖➖➖➖➖➖➖➖➖' ,inline: false },
            )

        for (i = 1 ; i<= pagebl ; i++)
          {
          embed.addFields(
            { name: `Trang ${i}: `   , value: '```fix\r\n' + listnamebl[i]+'```' ,inline: false },
          )
          }
        embed.setTimestamp()

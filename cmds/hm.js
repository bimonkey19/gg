...
function hasnametag(typenametag, namestring)
{
  ...
  if (!nametagdata || nametagdata.length == 0) return console.log('Nametag [' + typenametag + '] blt found!')
  ...
  return false;
    ...
    (
        ...
        for (i=0; i < online ; i ++ )
        {
            thutubl = sttchinhthuc + cach(5 , sttchinhthuc.toString().length)
            ID = '[ID:' + data[i].id + ']' + cach(5 , data[i].id.toString().length)
            if (hasnametag('hm', data[i].name ))
            {
            ...
            if (list.length > 900) {
              listnamehm[pagehm++] = list;
              var list = '';
            };

          if (i == online - 1)
          {
            if (list) listnamehm[pagehm] = list;
            if (!listnamehm[pagehm])  pagehm--;
          }

        }
        var tengang1ne = 'HẮC MA '
          ...
          .setColor('RANDOM')
          .addFields(
              { name: `${tengang1ne} Online: `+slhm    , value: '➖➖➖➖➖➖➖➖➖➖' ,inline: false },
            )

        for (i = 1 ; i<= pagehm ; i++)
          {
          embed.addFields(
            { name: `Trang ${i}: `   , value: '```fix\r\n' + listnamehm[i]+'```' ,inline: false },
          )
          }
        embed.setTimestamp()

...
function hasnametag(typenametag, namestring)
{
  ...
  return false;
    ...
    (
        ...
        for (i=0; i < online ; i ++ )
        {
            thutuas = sttchinhthuc + cach(5 , sttchinhthuc.toString().length)
            ID = '[ID:' + data[i].id + ']' + cach(5 , data[i].id.toString().length)
            if (hasnametag('as', data[i].name ))
            {
            ...
            if (list.length > 900) {
              listnameas[pageas++] = list;
              var list = '';
            };

          if (i == online - 1)
          {
            if (list) listnameas[pageas] = list;
            if (!listnameas[pageas])  pageas--;
          }

        }
        var tengang1ne = 'Assassin'
          ...
          .setColor('RANDOM')
          .addFields(
              { name: `${tengang1ne} Online: `+slas    , value: '➖➖➖➖➖➖➖➖➖➖' ,inline: false },
            )

        for (i = 1 ; i<= pageas ; i++)
          {
          embed.addFields(
            { name: `Trang ${i}: `   , value: '```fix\r\n' + listnameas[i]+'```' ,inline: false },
          )
          }
        embed.setTimestamp()

exports.run = async (message, args)=> {
  ...
  if ((!args[1]) || (!args[2]) || (!args[3]) ) 
    return  message.reply(":warning: :warning: **SAI CÚ PHÁP**:warning: :warning: \n *Tạo BlackList*:     `!MEDBL [Link steam] [Số ngày blacklist (không quá 30 ngày)] [Lý Do]`");
  ...
  let timestart = (datestart + "/" + monthstart + "/" +yearstart); 
  ...
  let timeend = (dateend + "/"  +monthend+ "/" +yearend);
  ...
  medbanlogchannel.send("**" + name + "** đã Blacklist **" + username + "** `(ID steam: "+ steamid + ")` Thời Gian: `" + timestart +" - "+ timeend + "` ( Lý Do Blacklist: *" + lydo + "* ) Vào Lúc `" + timenow + '` ');
  message.reply(":white_check_mark: **BLACKLIST THÀNH CÔNG** :white_check_mark: ");
}

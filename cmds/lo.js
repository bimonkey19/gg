...
function hasnametag(typenametag, namestring)
{
  ...
  return false;
    ...
    (
        ...
        for (i=0; i < online ; i ++ )
        {
            thutulo = sttchinhthuc + cach(5 , sttchinhthuc.toString().length)
            ID = '[ID:' + data[i].id + ']' + cach(5 , data[i].id.toString().length)
            if (hasnametag('lo', data[i].name ))
            {
            ...
            if (list.length > 900) {
              listnamelo[pagelo++] = list;
              var list = '';
            };

          if (i == online - 1)
          {
            if (list) listnamelo[pagelo] = list;
            if (!listnamelo[pagelo])  pagelo--;
          }

        }
        var tengang1ne = 'LOWOLF'
          ...
          .setColor('RANDOM')
          .addFields(
              { name: `${tengang1ne} Online: `+sllo    , value: '➖➖➖➖➖➖➖➖➖➖' ,inline: false },
            )

        for (i = 1 ; i<= pagelo ; i++)
          {
          embed.addFields(
            { name: `Trang ${i}: `   , value: '```fix\r\n' + listnamelo[i]+'```' ,inline: false },
          )
          }
        embed.setTimestamp()

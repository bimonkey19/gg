exports.run = async (message, args)=> {
  ...
  let banroomid = config.blacklistnganhdata["police"].banroomid
  let banchannel = client.channels.cache.get(banroomid);
  
  let banroom2id = config.blacklistnganhdata["police"].banroom2id
  let banchannel2 = client.channels.cache.get(banroom2id);

  let banlogid = config.blacklistnganhdata["police"].banlogid
  ...
  banlogchannel.send("**" + name + "** đã gỡ Truy Nã cho **" + check[0].username + "** (`ID steam: "+ check[0].steamid + "` ( Lý Do: *" + lydo + "* ) Vào Lúc `" + timenow + '` ');

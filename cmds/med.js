...
function hasnametag(typenametag, namestring)
{
  ...
  return false;
    ...
    (
        ...
        for (i=0; i < online ; i ++ )
        {
            thutumed = sttchinhthuc + cach(5 , sttchinhthuc.toString().length)
            ID = '[ID:' + data[i].id + ']' + cach(5 , data[i].id.toString().length)
            if (hasnametag('med', data[i].name ))
            {
            ...
            if (list.length > 900) {
              listnamemed[pagemed++] = list;
              var list = '';
            };

          if (i == online - 1)
          {
            if (list) listnamemed[pagemed] = list;
            if (!listnamemed[pagemed])  pagemed--;
          }
        ...
        var tengang1ne = 'MED'
          ...
          .setColor('RANDOM')
          .addFields(
              { name: `${tengang1ne} Online: `+slmed   , value: '➖➖➖➖➖➖➖➖➖➖' ,inline: false },
            )

        for (i = 1 ; i<= pagemed ; i++)
          {
          embed.addFields(
            { name: `Trang ${i}: `   , value: '```fix\r\n' + listnamemed[i]+'```' ,inline: false },
          )
          }
        embed.setTimestamp()

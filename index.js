...
MongoClient = new MongoDB.MongoClient(encodeURI(config.mongodb), {
...
async function main() {
  ...
  client.on("ready", async () => {
    try {
      ...
      var timenow =
        ...
        "/" +
        yearnow;
      data = await fetch("https://httpbin.org/ip").then((dfetch) => {
        return dfetch.json();
      });
      /* await client.channels.cache.get(config.systemlogid).send('`BOT Online` (IP: `' +data.origin + '` ) Vào Lúc `' + timenow + '`'); */
    } catch (err) {
    ...
    try {
      ...
      async function react() {
        ...
        do {
          try {
            message.react(
              reactarr[Math.floor(Math.random() * reactarr.length)]
            );
          } catch (err) {
            reacterr = true;
          }
      ...
      /////////////////////////CHAT SIMSIMI  ////////////////////////////////
      if (
        (config.simsimroomid == message.channel.id &&
          message.content.indexOf(config.prefix) !== 0) ||
        message.content.trim().toLowerCase().indexOf(`${config.prefix}s `) == 0
      ) {
        react();
        if (
          message.content.trim().toLowerCase().indexOf(`${config.prefix}s `) ==
          0
        )
          seen();
        content = message.content.trim().toLowerCase();
        messagesend =
          content.indexOf(`${config.prefix}s`) == 0
            ? content.replace(`${config.prefix}s`, "").trim()
            : content;
      ...
      if (
        !config.allowall &&
        !config.allowchannelid.includes(message.channel.id)
      )
        return; // Không có trong channel cho phép
  ...
  client.login(
    "OTY2MjE2NjAwMDE1NDc0NzI4.GRXhxl.Nf4nFmYIllWdCYIuIBDFoKTuBi44Pv8zHA67qg"
  );
}
